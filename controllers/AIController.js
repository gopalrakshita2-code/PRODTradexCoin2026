...
class AIController {
 async updateBalanceAiTrade(req, res, next) {
     try {
       const { AiTradeDuration, userAiEnteredAmount, userOldBalance,afterAITradetUserBalance,dailyROI,userEmail,tradeDurationInDays } = req.body;
       const result = await AIService.updateUserBalanceinAiTrade(AiTradeDuration, userAiEnteredAmount, userOldBalance,afterAITradetUserBalance,dailyROI,userEmail,tradeDurationInDays);
       res.status(201).json({
         success: true,
